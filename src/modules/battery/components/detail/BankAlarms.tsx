...
export const BankAlarms = ({bankId}: {bankId: string}) => {
  ...
  return (
    <div className="flex flex-col gap-6 px-4 pt-2 pb-8">
      ...
      {/* What the unit watches against what it is saying, so the site tab's
          cross-reference reconciles with this page. The five it will not assert are
          named because their absence is a deliberate rule rather than luck: each one
          claims load is off the air, and every other screen for this site says it is
          up and generating. */}
      {watched > 0 && (
