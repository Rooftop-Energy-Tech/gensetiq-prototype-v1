 ...
 *
 * There are no battery *alarms* in this model, so hours left is the thing here most
 * likely to need somebody tonight, and a register that opened alphabetically would
 * bury the flat bank at position nineteen. `/gensets` and `/sites` both sort by
 ...
 * the sort.** It moves over years rather than hours, so a health-sorted register would
 * hand back the same order every morning — a ranking with no news in it — while the
 * runtime sort changes through the day and is what the page is opened for.
 */
