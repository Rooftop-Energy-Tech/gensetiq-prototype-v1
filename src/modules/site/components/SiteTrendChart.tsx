...
export const SiteTrendChart = ({
  ...
  return (
    <div ref={boxRef} className={cn('relative w-full', colorClassName)}>
      ...
      {/* The split as shares over time — the strip under the chart, same spine,
          one line per source. The table it replaced is described on
          `SiteTrend.mixTrend`. */}
      {trend.mixTrend !== undefined && (
        <ShareTrend series={trend.mixTrend} bars={bars} ariaLabel="Share per bucket, in percent" />
      )}
    </div>
  );
};
