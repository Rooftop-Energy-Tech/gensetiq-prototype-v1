 ...
 * generate nothing, and including them would dilute the programme's own figures
 * with a pile of towers in towns it was never about.
 *
 * ## What this page no longer does
 *
 * It used to carry the money as well — a delivered diesel rate, a saving a year,
 * payback, ROI, and a quotation for every site still on diesel — and underneath
 * that, the litres each site *would* have burned running on diesel alone. All of
 * it has been taken out and will come back as its own thing.
 *
 * What is left is a measurement: what carried the load, how long the engines ran,
 * and what they burned. No counterfactual, no rate, no verdict on whether the
 * programme is paying.
 *
  ...
  const rows = useMemo(
    () =>
      summaries
        ...
        // Thirstiest first. The table used to be sorted on payback, which was the
        // right column to read it down while the money was here; with that gone
        // the useful order is what each site is actually burning, because that is
        // what the tanker runs and the conversion queue both follow.
        .sort((left, right) => right.energy.litres - left.energy.litres),
    [summaries, roles],
  );
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-auto px-4 pt-3 pb-24 md:pb-6">
      ...
      <section aria-label="By site" className="flex min-h-0 min-w-0 flex-col gap-2">
        <header className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
          ...
          <p className="text-xs text-tertiary">
            Thirstiest first. A site still on diesel prime has no plant to report, so its
            row is the genset alone
          </p>
        </header>
        ...
        <div className="min-h-0 overflow-auto">
          <table className="w-full table-fixed border-separate border-spacing-0 text-sm">
            <caption className="sr-only">
              Every off-grid site, its plant, what carried it over thirty days and what its
              genset burned
            </caption>
            <colgroup>
              {COLUMNS.map((column) => (
            ...
            <tbody>
              {filtered.map(({summary, role, energy}) => {
                return (
