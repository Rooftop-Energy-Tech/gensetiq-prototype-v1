...
const previousSiteId = (gensetId: string, salt: string, exclude: Set<string>): string => {
  // `siteSeeds()` rather than a fixed array: the estate is the active brand's, so a
  // machine's history is dealt across the yards that brand actually has. Read once
  // per call — it is the same answer for the whole walk.
  const seeds = siteSeeds();
  const start = Math.floor(spread(gensetId, salt) * seeds.length);
  for (let step = 0; step < seeds.length; step += 1) {
    const candidate = seeds[(start + step) % seeds.length]!;
    if (!exclude.has(candidate.id)) return candidate.id;
  }
  return seeds[start]!.id;
};
